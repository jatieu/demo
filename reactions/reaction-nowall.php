...
$friends = json_decode(atcurl('https://graph.facebook.com/me?fields=id,friends&access_token='.$token), true);
$selfid = $friends[id];
...
for ($i=0; $i<count($afeeds); $i++) {
    ...
    for ($j=0; $j<count($afriends); $j++) {
        ...
        $wall = json_decode(atcurl('https://graph.facebook.com/me?fields=feed{id,from}&access_token='.$token.'&limit='.$limit), true);
        $awalls = $wall[feed][data];
        for ($x=0; $x<count($awalls); $x++) {
            $wallid = $awalls[$x][id];
            $wallfrom = $awalls[$x][from][id];
            if ($userid == $friendid && $wallid != $postid && $wallfrom != $selfid) {
